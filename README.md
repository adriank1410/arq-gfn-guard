...
When a verified append moves event timestamps backwards, the guard uses that source until its session ends or GFN exits. After the end, superseded evidence from the other file stays excluded until a new event appears; later sessions can use either source. Both the selected clock source and excluded old events survive a restart even after backups have resumed, in the independent `guard-clock` file. Versioned clock snapshots and lease evidence carry a monotonic revision so an older snapshot cannot overwrite a newer saved lease after a partial write failure. Losing that source raises an alert and preserves the owned pause. A clock change before the guard starts cannot be reliably inferred from logs without time zones. After an observed GFN exit, old start events cannot pause backups again merely because the launcher reopens.
...
Successive authenticated rotations retain the last confirmed session state even after its original event leaves both the log and `.bak`. Replacement checkpoints and the identity of an authenticated rotated writer are saved without extending the Arq lease, and are revalidated after a guard restart. An end recorded only in `.bak` also survives a later restart without replaying the ended session. The authenticated writer remains tracked after resume so it can report a later new session, even while the primary file has no lifecycle records. Unrelated backups remain untrusted. An unavailable/untrusted-log warning describes uncertainty about the logs; when the guard owns a pause, it reports that it is still trying to maintain it, rather than implying that the pause failed.
